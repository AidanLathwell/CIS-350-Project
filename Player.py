...
class Player:
    ...
    def split(self, deck):
        ...
        iterations = 0
        if self.already_hit is False and len(self.hand.hand) == 2 and iterations == 0:
            iterations += 1

            split_card = self.hand.hand.pop(1)
            self.new_hand.hand.append(split_card)

            # add card to new hand
            card_for_hand = deck.hit()
            if card_for_hand:
                self.new_hand.hand.append(card_for_hand)

            self.new_hand.calc_hand_value()
            self.new_hand.calc_ability_to_hit()
